        ...
        <CardBody>
          ...
          {selectedOffering ? (
            <div className="flex flex-wrap items-center justify-between gap-3">
              <span className="text-sm font-medium text-neutral-900">{offeringLabel(selectedOffering)}</span>
              <Link href="/admin/registrations" className="text-sm font-medium text-brand-700 hover:underline">
            ...
            <>
              ...
              <ul className="flex flex-col gap-1.5">
                {pagedOfferings.map((o) => (
                  <li key={o.id} className="flex items-center justify-between gap-3 rounded-md border border-neutral-200 px-3 py-2 text-sm">
                    <span>{offeringLabel(o)}</span>
                    <Link href={hrefWith({ offeringId: o.id })} className="shrink-0 font-medium text-brand-700 hover:underline">
                      Open
                    </Link>
                  </li>
                ))}
        ...
        <>
          <Card className="mb-6">
            <CardBody>
              ...
              {!candidates && <p className="text-sm text-neutral-500">Search for a student to register them directly.</p>}
              {candidates && candidates.rows.length === 0 && (
                <p className="text-sm text-neutral-500">No active students match &ldquo;{sq}&rdquo;.</p>
              )}
                    ...
                    <li key={s.id} className="rounded-md border border-neutral-200 px-3 py-2">
                      <form action={registerDirectAction} className="flex flex-wrap items-end gap-2">
                        ...
                        <span className="mb-1.5 grow text-sm">
                          <span className="font-mono text-xs text-neutral-600">{s.studentNumber}</span> — {s.firstName} {s.lastName}
                        </span>
                        <div>
                  ...
                  <Card className="flex items-center justify-between px-3 py-2 text-sm">
                    ...
                    {r.status === "REGISTERED" && (
                      <form action={dropRegistrationAction} className="flex items-center gap-2">
                        ...
                        <SubmitTextButton pendingLabel="Dropping…" className="text-xs font-medium text-danger-600 hover:underline">
                          Drop
                        </SubmitTextButton>
                      </form>
                    )}
