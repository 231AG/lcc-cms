...
> **Correction, 19 Sep 2026.** The evidence cell above claimed more than the test behind it. `identity_rls.integration.test.ts` asserts RLS on **`app_user` only** — which roles each actor can see rows for. It does not touch `getGradeSheet`, `getStudent`, `getCumulativeSummary`, `getSemesterSummaries` or `getStudentHistory`, so "request student B's record" was never actually exercised against a transcript, a GPA or an academic record.
>
> The claim was also load-bearing in a way worth naming: those readers take a `studentId` as an ordinary argument with **no service-layer check that it belongs to the caller**, so RLS is the entire boundary, and nothing asserted it held.
>
> `src/lib/permissions/__tests__/crossStudentAccess.integration.test.ts` now does: two real students, both with grades, student A asking for student B's records through each reader in turn. It was verified to **fail** when one reader is switched from `asUser()` to the superuser connection, so it is testing RLS rather than passing incidentally.
